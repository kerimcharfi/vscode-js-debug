<script>
  ...
  const main = async () => {
    ...
    const runtimeInstantiation = await instantiateWASM(buf, {repl, memory,__indirect_function_table: table});
    ...
    const { wasi, instance, string, getMem } = await instantiateWASM(buf, {repl, memory,__indirect_function_table: table, ...runtimeInstantiation.instance.exports,
      // __table_base: 35000, 
      _swift_FORCE_LOAD_$_swiftWASILibc: new WebAssembly.Global({ value: "i32", mutable: true }, 0),
      __start_swift5_accessible_functions: new WebAssembly.Global({ value: "i32", mutable: true }, 0),
      __stop_swift5_accessible_functions: new WebAssembly.Global({ value: "i32", mutable: true }, 0),
    ...
    const replInstatiation = await instantiateWASM(buf, {repl, memory, __indirect_function_table: table, ...instance.exports, ...runtimeInstantiation.instance.exports, 
      // __table_base: 35000, 
      _swift_FORCE_LOAD_$_swiftWASILibc: new WebAssembly.Global({ value: "i32", mutable: true }, 0),
      __start_swift5_accessible_functions: new WebAssembly.Global({ value: "i32", mutable: true }, 0),
      __stop_swift5_accessible_functions: new WebAssembly.Global({ value: "i32", mutable: true }, 0),
