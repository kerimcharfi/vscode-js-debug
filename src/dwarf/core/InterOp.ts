...
import { WasmValue, WasmValueVector } from '../pkg';

export async function createWasmValueStore(
  runtime: ProtocolApi.RuntimeApi,
  data: Protocol.Runtime.PropertyDescriptor[],
) {
  ...
  await Promise.all(
    data.map(async x => {
      ...
      const value =
        (result.result[1].value!.value as number) ||
        (result.result[1].value!.unserializableValue?.slice(0, -1) as string);
