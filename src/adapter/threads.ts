...
export class Thread implements IVariableStoreLocationProvider {
  ...
  public scriptLocationToUiLocation(
    ...
    if (!scriptLocation.scriptId || this.disposed) {
      return undefined;
    }
    ...
    if (!script) {
      if (!scriptLocation.scriptId) {
        return;
      }

      return this.waitForScriptId(scriptLocation.scriptId).then(async script => {
        if(!script) return

        await script.sourcesPromise;
        return this.sourceContainer.preferredUiLocation({
          ...rawToUiOffset(scriptLocation, script.runtimeScriptOffset),
          script,
        });
  ...
  private _onScriptParsed(event: Cdp.Debugger.ScriptParsedEvent) {
    ...
    script._container = this.sourceContainer
    script.executionContextId = event.executionContextId,
      // source: createSource(),

    script.sourcesPromise = this.getSourcesAndMapsFromScriptEvent(event, script).then(({sources, sourceMap})=>{
  ...
  async getSourcesAndMapsFromScriptEvent(event: Cdp.Debugger.ScriptParsedEvent, script: Script): Promise<{sources:Source[], sourceMap: SourceMap}>{
    ...
    if (event.sourceMapURL) {
      ...
      return await this._getOrLoadSourceMaps(event, script)
    }

    // assuming plain js without sourcemap -> create Source from scripts contents
    let source = this.sourceContainer.getSourceByOriginalUrl(event.url);
    if (!source || !event.hash || source?.contentHash !== event.hash) {
      source = await Source.createFromScript(event, this, script)
    }

    return {
      sources: [source],
      sourceMap: new DummySourceMap(source, script)
    }
  ...
  async _handleSourceMapPause(
    ...
    const result = await Promise.race([this._getOrLoadSourceMaps(script), delay(timeout)]);
   ...
   * handler's results.
   */
  private async _getOrLoadSourceMaps(event: Cdp.Debugger.ScriptParsedEvent, script: Script): Promise<{sources:Source[], sourceMap: SourceMap}> {
    const noResult = {sources: [], sourceMap: undefined}
    const ctx = this._executionContexts.get(event.executionContextId);
    if (!ctx) {
      return noResult;
    }
    ...
    // this.sourceContainer.scriptSkipper.initializeSkippingValueForSource(source);
    deferred.resolve();
